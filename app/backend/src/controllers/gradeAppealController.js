...
const respondGradeAppeal = async (req, res, next) => {
    try {
        const {grade_appeal_id: gradeAppealId, reply_details: replyDetails, student_id: studentId, examId} = req.body;
        if (!gradeAppealId || !replyDetails || !studentId || !examId) {
            return res.status(400).json({message: "Missing required fields"});
        }
...
module.exports = {submitGradeAppeal, respondGradeAppeal, fetchStudentResolvedGradeAppeals, fetchStudentUnresolvedGradeAppeals };